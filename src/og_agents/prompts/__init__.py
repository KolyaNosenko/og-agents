from src.og_agents.prompts.generate_competency_questions_prompt import GenerateCompetencyQuestionsPrompt
from src.og_agents.prompts.generate_ontology_prompt import GenerateOntologyPrompt
from src.og_agents.prompts.oops_validation_result_prompt import OOPSValidationResultPrompt
from src.og_agents.prompts.ontology_consistency_validation_result_prompt import OntologyConsistencyValidationResultPrompt

__all__ = [
    "GenerateCompetencyQuestionsPrompt",
    "GenerateOntologyPrompt",
    "OOPSValidationResultPrompt",
    "OntologyConsistencyValidationResultPrompt"
]
